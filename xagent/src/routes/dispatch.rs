...
pub fn dispatch_run_async(
    ...
    match enqueue_job_idempotent(
        ...
        Some((job_id, replayed)) => {
            ...
            ok(
                "dispatched",
                json!({ "id": job_id, "host_id": host_id, "action": action_id, "priority": priority, "replayed": replayed, "idempotency_key": idem.0 }),
            )
        }
...
pub fn dispatch_fanout(
    ...
    let host_ids: Vec<String> = {
        ...
        match &body.hosts {
            Some(ids) => ids
                .iter()
                .filter(|id| inner.hosts.iter().any(|h| &h.id == *id))
                .cloned()
                .collect(),
            None => inner
                .hosts
    ...
    ok(
        "fanout_dispatched",
        json!({ "dispatched": dispatched, "count": dispatched.len(), "replayed": false, "idempotency_key": idem.0 }),
    )
}
...
pub fn dispatch_job_retry(
    ...
    match retry_job(state, &body.id, "api:dispatch-job-retry") {
        ...
        Err(_) => err(
            "retry_failed",
            "Dispatch job could not be retried.",
            "conflict",
        ),
    }
}
