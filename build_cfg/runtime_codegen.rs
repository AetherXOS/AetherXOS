//! Scans src/config/ for runtime setters and generates the key catalog.
...
fn scan_runtime_setters() -> Vec<RuntimeSetterSpec> {
    let mut files = vec!["src/config.rs".to_string()];
    list_rs_files(Path::new("src/config"), &mut files);
...
fn scan_enum_variants() -> BTreeMap<String, Vec<String>> {
    ...
    let raw = match fs::read_to_string("src/config/parsers.rs") {
        Ok(v) => v,
        Err(_) => return out,
    };
...
pub fn generate_runtime_key_autogen() {
    ...
    content
        .push_str("//! Generated by build.rs from set_* Option<T> setters under src/config*\n\n");
    ...
    fs::write("src/config/runtime_key_autogen.rs", content)
        .expect("Failed to write src/config/runtime_key_autogen.rs");
}
