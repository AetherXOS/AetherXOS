...
impl core::str::FromStr for KernelFeatures {
    ...
    fn from_str(s: &str) -> core::result::Result<Self, Self::Err> {
        Ok(Self::from_cargo_features(
            &s.split(',').collect::<alloc::vec::Vec<_>>(),
        ))
    }
}
...
impl KernelFeatures {
    pub fn to_cargo_features(self) -> alloc::vec::Vec<&'static str> {
        ...
        if self.contains(Self::VFS) {
            features.push("vfs");
        }
        if self.contains(Self::DRIVERS) {
            features.push("drivers");
        }
        if self.contains(Self::NET) {
            features.push("net");
        }
        if self.contains(Self::SMP) {
            features.push("smp");
        }
        if self.contains(Self::TEST_MODE) {
            features.push("kernel_test_mode");
        }
        if self.contains(Self::GRAPHICS) {
            features.push("graphics");
        }
        if self.contains(Self::USB) {
            features.push("usb");
        }
        if self.contains(Self::PCI) {
            features.push("pci");
        }
        if self.contains(Self::ACPI) {
            features.push("acpi");
        }
        if self.contains(Self::LOGGING) {
            features.push("logging");
        }
        if self.contains(Self::SCHEDULER) {
            features.push("scheduler");
        }
        if self.contains(Self::MEMORY_MGMT) {
            features.push("memory_mgmt");
        }
        if self.contains(Self::SYSCALLS) {
            features.push("syscalls");
        }
        if self.contains(Self::KVM) {
            features.push("kvm");
        }
        if self.contains(Self::HARDENING) {
            features.push("hardening");
        }
        if self.contains(Self::IO_URING) {
            features.push("io_uring");
        }
        if self.contains(Self::DEBUG_TEST_OUTPUT) {
            features.push("debug_test_output");
        }
        features
    }
