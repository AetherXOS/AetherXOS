...
pub fn config_overrides_template(_role: RequireViewer) -> Value {
    ok("config_overrides_template", json!({
        "template": {
            "agent": {
                "port": 7401,
                "auth_mode": "strict",
...
pub fn config_compose_apply(_role: RequireAdmin) -> Value {
    ok("config_compose_applied", json!({
        "applied_utc": Utc::now().to_rfc3339(),
    }))
}
...
fn load_config_file() -> Value {
    let candidates = [
        "scripts/config/hypercore.defaults.json",
        "../scripts/config/hypercore.defaults.json",
        "../../scripts/config/hypercore.defaults.json",
    ];
    for p in &candidates {
