...
fn run_inner(state: &State<AppState>, role: &str, idem_key: Option<&str>, body: &RunPayload, source: &str) -> Value {
    ...
    match enqueue_job_idempotent(state, source, idem_key, &action_id, &priority, source) {
        Some((job_id, replayed)) => {
            ...
            ok("queued", json!({ "id": job_id, "action": action_id, "priority": priority, "replayed": replayed, "idempotency_key": idem_key }))
        }
