...
pub fn create_partitioned_raw_image_from_dir(src_dir: &Path, img_out: &Path) -> Result<()> {
    ...
    if cfg!(windows) && process::which("wsl") {
        logging::info(
            "build::image",
            "Windows detected; using WSL for partitioned image creation",
            &[],
        );
...
"#,
        ...
        match wsl::run_in_wsl(
            ...
            Ok(_) => {
                logging::info(
                    "build::image",
                    "Partitioned image created successfully in WSL",
                    &[],
                );
                return Ok(());
            }
            Err(e) => {
                logging::warn(
                    "build::image",
                    &format!(
                        "WSL image creation failed: {}; falling back to simple copy",
                        e
                    ),
                    &[],
                );
    ...
    if !process::which("parted") {
        return Err(anyhow!("'parted' not found on host"));
    }
    if !process::which("losetup") {
        return Err(anyhow!("'losetup' not found on host"));
    }
    if !process::which("kpartx") {
        return Err(anyhow!("'kpartx' not found on host"));
    }
    if !process::which("mkfs.ext4") {
        return Err(anyhow!("'mkfs.ext4' not found on host"));
    }

    process::run_checked(
        "parted",
        ["-s", img_out.to_string_lossy().as_ref(), "mklabel", "msdos"],
    )?;
    process::run_checked(
        "parted",
        [
    ...
    if !losetup_out.status.success() {
        return Err(anyhow!("losetup failed"));
    }
    ...
    let loop_base = Path::new(&loop_dev)
        .file_name()
        .unwrap()
        .to_string_lossy()
        .into_owned();
    ...
    process::run_checked(
        "mount",
        [mapped_part.as_str(), mount_point.to_string_lossy().as_ref()],
    )?;
    ...
    process::run_checked(
        "sh",
        ["-c", format!("{} | {}", tar_cmd, extract_cmd).as_str()],
    )?;
