...
pub fn check_registry_integrity(file: Option<&str>) -> Result<()> {
    ...
    println!(
        "[distro::check] Validating integrity of registry v{}",
        registry.version
    );
    ...
    println!(
        "[distro::check] Integrity check PASSED ({} total image variants found)",
        images.len()
    );
    Ok(())
}
...
pub fn validate_urls(file: Option<&str>, out: Option<&str>, _verify_size: bool) -> Result<()> {
                    ...
                    if let Some(len_header) = resp.headers().get(reqwest::header::CONTENT_LENGTH) {
                        if let Ok(s) = len_header.to_str() {
                            if let Ok(size) = s.parse::<u64>() {
                                ...
                                if size < min_size {
                                    println!(
                                        "[FAIL] URL too small (<5MB): {} (got {} bytes)",
                                        url, size
                                    );
                                    total_failed += 1;
                                }
    ...
    if total_failed > 0 {
        println!(
            "[distro::url-checks] BATCH FAILED: {} issues found. See {}",
            total_failed,
            report_path.display()
        );
    } else {
