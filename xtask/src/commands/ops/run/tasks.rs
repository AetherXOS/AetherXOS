...
impl Task for SmokeTestTask {
    fn name(&self) -> String {
        "QEMU Smoke Test".to_string()
    }
    fn description(&self) -> String {
        "Runs an automated smoke test in QEMU to verify bootability".to_string()
    }

    fn run(&self, _ctx: &ExecutionContext) -> Result<TaskStatus> {
        logging::info(
            "SMOKE",
            "Initializing automated smoke test sequence",
            &[("bootloader", self.bootloader.as_str())],
        );
