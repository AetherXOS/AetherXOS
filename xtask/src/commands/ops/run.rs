...
pub fn execute(action: &RunAction) -> Result<()> {
    match action {
        RunAction::Smoke { bootloader } => {
            println!(
                "[run::smoke] Starting automated generic smoke test via headless QEMU parameter constraints."
            );
            println!(
                "[run::smoke] Intercepted intended target bootloader sequence: {:?}",
                bootloader
            );
        ...
        RunAction::Live { firmware } => {
            println!(
                "[run::live] Launching interactive QEMU graphic interface utilizing firmware mode: '{}'.",
                firmware
            );
        ...
        RunAction::Debug { firmware } => {
            ...
            println!(
                "[run::debug] Firmware parameters '{}' active. Proceeding to inject TCP debugging port (tcp::1234)...",
                firmware
            );
        ...
        RunAction::PxeServer { port } => {
            println!(
                "[run::pxe] Emulating stateless distribution host for physical machine broadcasting."
            );
            println!(
                "[run::pxe] Initializing HTTP-based iPXE delivery subsystem natively on port: {}",
                port
            );
            ...
            println!(
                "[run::pxe] Emulating dynamic TFTP/HTTP serving at '{}' to generic physical Network Cards (NIC).",
                host_dir.display()
            );
            println!(
                "[run::pxe] Use your other computer's BIOS -> Network Boot pointing to this host's IP/{}",
                port
            );
        ...
        RunAction::BareMetalDeploy { device } => {
            ...
            println!(
                "[run::deploy] CRITICAL PROCESS: Immediate raw byte-transfer to '{}' target initialized.",
                device
            );
            println!(
                "[run::deploy] CAUTION: This operation bypasses logical FAT formats to enforce raw DD block sector overwrites."
            );
            ...
            println!(
                "[run::deploy] Origin block source verified: {}",
                img_path.display()
            );
            println!(
                "[run::deploy] Requesting administrative lock overlay on target physical address: {}",
                device
            );
            println!(
                "[run::deploy] Stub constraint triggered. (Requires explicitly elevated Host OS execution context to open raw PhysicalDrive)."
            );
        }
    }
