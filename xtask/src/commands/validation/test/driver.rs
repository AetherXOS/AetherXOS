...
pub fn run_smoke() -> Result<()> {
    ...
    let mapping: &[(&str, &str)] = &[
        (
            "network_irq_service_budget",
            "DRIVER_NETWORK_IRQ_SERVICE_BUDGET",
        ),
        (
            "network_loop_service_budget",
            "DRIVER_NETWORK_LOOP_SERVICE_BUDGET",
        ),
        ("network_ring_limit", "DRIVER_NETWORK_RING_LIMIT"),
        (
            "network_quarantine_rebind_failures",
            "DRIVER_NETWORK_QUARANTINE_REBIND_FAILURES",
        ),
        (
            "network_quarantine_cooldown_samples",
            "DRIVER_NETWORK_QUARANTINE_COOLDOWN_SAMPLES",
        ),
        (
            "network_slo_max_drop_rate_per_mille",
            "DRIVER_NETWORK_SLO_MAX_DROP_RATE_PER_MILLE",
        ),
        (
            "network_slo_max_tx_ring_utilization_percent",
            "DRIVER_NETWORK_SLO_MAX_TX_RING_UTILIZATION_PERCENT",
        ),
        (
            "network_slo_max_rx_ring_utilization_percent",
            "DRIVER_NETWORK_SLO_MAX_RX_RING_UTILIZATION_PERCENT",
        ),
        (
            "network_slo_max_io_errors",
            "DRIVER_NETWORK_SLO_MAX_IO_ERRORS",
        ),
        ("ahci_io_timeout_spins", "DRIVER_AHCI_IO_TIMEOUT_SPINS"),
        (
            "nvme_disable_ready_timeout_spins",
            "DRIVER_NVME_DISABLE_READY_TIMEOUT_SPINS",
        ),
        ("nvme_poll_timeout_spins", "DRIVER_NVME_POLL_TIMEOUT_SPINS"),
        ("nvme_io_timeout_spins", "DRIVER_NVME_IO_TIMEOUT_SPINS"),
        (
            "e1000_reset_timeout_spins",
            "DRIVER_E1000_RESET_TIMEOUT_SPINS",
        ),
        ...
        match meta_val {
            Some(m_val) => {
                ...
                match gen_val {
                    ...
                    Some(g_val) => failures.push(format!(
                        "mismatch {}/{}: expected {}, got {}",
                        key, const_name, m_val, g_val
                    )),
    ...
    if failures.is_empty() {
        ...
        for failure in &failures {
            logging::warn("test", "driver smoke mismatch", &[("detail", failure)]);
        }
