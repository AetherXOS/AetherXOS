...
pub fn run() -> Result<()> {
    println!("[status::tier] Generating P0/P1/P2 aggregated readiness status");
    ...
    // P0 Checks
    let p0_checks = vec![
        bool_check(&root, "health_score", vec!["reports/tooling/health_report.json"], true, |d| {
            let score = d.get("score").and_then(|v| v.as_f64()).unwrap_or(0.0);
            (score >= 60.0, format!("score={score}"))
        }, "missing health_report"),
        bool_check(&root, "policy_gate", vec!["reports/tooling/policy_gate.json"], true, |d| {
            (d.get("ok").and_then(|v| v.as_bool()).unwrap_or(false), "policy_gate.ok".to_string())
        }, "missing policy gate"),
        bool_check(&root, "syscall_default", vec![config::repo_paths::SYSCALL_COVERAGE_SUMMARY], true, |d| {
            let pct = d.get("implemented_pct").and_then(|v| v.as_f64()).unwrap_or(0.0);
            (pct >= 100.0, format!("implemented_pct={pct}"))
        }, "missing syscall coverage summary"),
        bool_check(&root, "syscall_linux_compat", vec!["reports/syscall_coverage_linux_compat_summary.json"], true, |d| {
            let pct = d.get("implemented_pct").and_then(|v| v.as_f64()).unwrap_or(0.0);
            (pct >= 100.0, format!("implemented_pct={pct}"))
    ...
    // P1 Checks
    let p1_checks = vec![
        bool_check(&root, "posix_conformance", vec![config::repo_paths::POSIX_CONFORMANCE_SUMMARY], true, |d| {
            let ok = d.get("summary").and_then(|v| v.get("ok")).and_then(|v| v.as_bool()).unwrap_or(false);
            (ok, "summary.ok".to_string())
        }, "missing posix conformance summary"),
        bool_check(&root, "soak_stress_chaos", vec!["reports/soak_stress_chaos.json"], true, |d| {
            let ok = d.get("summary").and_then(|v| v.get("ok")).and_then(|v| v.as_bool()).unwrap_or(false);
            (ok, "summary.ok".to_string())
    ...
    // P2 Checks
    let p2_checks = vec![
        bool_check(&root, "p2_gap_gate", vec!["reports/p2_gap/gate_summary.json"], true, |d| {
            let ok = d.get("summary").and_then(|v| v.get("ok")).and_then(|v| v.as_bool()).unwrap_or(false);
            (ok, "summary.ok".to_string())
    ...
    let overall_completion_pct = if required_total > 0 { (required_passed as f64 / required_total as f64) * 100.0 } else { 100.0 };
    ...
    for tier_name in &["p0", "p1", "p2"] {
        if let Some(t) = tiers.get(*tier_name) {
            md.push_str(&format!("## {} - {}\n", tier_name.to_uppercase(), if t.ok { "OK" } else { "FAIL" }));
            md.push_str(&format!("- score_pct: {:.1}%\n", t.score_pct));
            md.push_str(&format!("- required_passed: {}/{}\n\n", t.required_passed, t.required_total));
            for c in &t.checks {
                md.push_str(&format!("- [{}] `{}` ({}) - {}\n", if c.ok { "x" } else { " " }, c.id, if c.required { "required" } else { "optional" }, c.detail));
            }
            md.push('\n');
        }
    }
    ...
    println!("[status::tier] DONE completion={:.1}% blockers={}", overall_completion_pct, blockers.len());
    Ok(())
}

fn bool_check<F>(root: &Path, id: &str, patterns: Vec<&str>, required: bool, predicate: F, missing_detail: &str) -> TierCheck 
where F: Fn(&serde_json::Value) -> (bool, String) 
{
        ...
        if path.exists() {
            if let Ok(text) = fs::read_to_string(&path) {
                if let Ok(json) = serde_json::from_str(&text) {
                    ...
                    return TierCheck {
                        id: id.to_string(), ok, required, detail, path: pattern.to_string(),
                    };
                }
            }
        }
    ...
    TierCheck {
        id: id.to_string(), ok: !required, required, detail: missing_detail.to_string(), path: String::new(),
    }
}
...
fn summarize_tier(checks: Vec<TierCheck>) -> TierSummary {
    ...
    let score = if req_total > 0 { (req_passed as f64 / req_total as f64) * 100.0 } else { 100.0 };
    TierSummary {
    ...
    for name in &["p0".to_string(), "p1".to_string(), "p2".to_string()] {
        ...
        if let Some(p) = &prev {
            if let Some(pt) = p.tiers.get(name) {
                ...
                if t.regression { trend.overall_regression = true; }
            }
        }
