...
fn main() -> Result<()> {
    ...
    if args_vec.len() == 1 {
        LAYOUT
            .ensure_hermetic()
            .context("Failed to stabilize project layout")?;
        app_context::init(LAYOUT.artifacts.clone())
            .context("Failed to initialize xtask runtime context")?;
        utils::preflight::NexusDoctor::new()
            .audit(true)
            .context("Nexus Doctor: System health audit failed")?;
    ...
    let is_info_command = matches!(
        &args.command,
        cli::Commands::LinuxAbi { .. }
            | cli::Commands::Glibc { .. }
