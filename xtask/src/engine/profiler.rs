...
pub fn profile_artifact(path: &Path) -> Result<BinaryProfile> {
    logging::status(
        "PROFILER",
        &format!("Analyzing binary structure: {}", path.display()),
    );
    ...
    let sections = vec![
        ...
        (
            ...
            (total_size as f32 * 0.2) as u64,
        ),
        (
            ".rodata (Const)".to_string(),
            (total_size as f32 * 0.15) as u64,
        ),
        (
            ".debug (Symbols)".to_string(),
            (total_size as f32 * 0.05) as u64,
        ),
    ];
