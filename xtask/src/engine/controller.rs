use crate::engine::{BuildProfile, ExecutionContext};
use anyhow::Result;
...
impl UniversalController {
    pub fn build_pipeline(
    ...
    ) -> Result<crate::engine::dag::DagPipeline> {
        ...
        match name {
            FULL_ISO => {
                ...
                dag.add_task(
                    Box::new(crate::engine::ResourceAuditTask),
                    vec!["Environment Audit"],
                );
                dag.add_task(
                    Box::new(crate::commands::infra::build::tasks::KernelCompileTask {
                        arch: crate::constants::defaults::build::ARCH,
                        release: ctx.is_release,
                        features: crate::utils::features::kernel_features_from_default(&[
                            "vfs", "drivers",
                        ])?,
                    }),
                    vec!["Resource Audit"],
                ...
                dag.add_task(
                    Box::new(crate::commands::validation::KernelSafetyAuditTask),
                    vec!["Kernel Compilation"],
                );
            ...
            DOCS => {
                dag.add_task(
                    Box::new(crate::engine::DocsGenerateTask {
            ...
            DEBUG => {
                dag.add_task(
                    Box::new(crate::engine::DebugBridgeTask {
    ...
    pub fn dispatch_workflow(name: &str, ctx: &ExecutionContext) -> Result<()> {
        crate::utils::ui::logging::aop_wrap_result(
            "DISPATCH",
            &format!("Orchestrating workflow: {}", name),
            || {
