...
impl Task for DownloadTask {
    fn name(&self) -> String { "Network Resource Acquisition".to_string() }
    fn description(&self) -> String { "Downloads external dependencies or artifacts from a remote URL".to_string() }
    ...
    fn run(&self, _ctx: &ExecutionContext) -> Result<TaskStatus> {
        ...
        crate::utils::sys::process::Executor::new(tool)
            .args(&["-fsSL", "--progress-bar", "-o", &self.dest.to_string_lossy(), &self.url])
            .run()
