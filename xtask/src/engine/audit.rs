...
impl Task for ToolchainAuditTask {
    fn name(&self) -> String {
        "Toolchain Audit".to_string()
    }
    fn description(&self) -> String {
        "Ensures all required Rust targets and components are installed".to_string()
    }
    ...
    fn run(&self, _ctx: &ExecutionContext) -> Result<TaskStatus> {
        ...
        if !installed.contains("x86_64-unknown-none") {
            logging::warn(
                "AUDIT",
                "Target 'x86_64-unknown-none' is missing. Attempting auto-installation...",
                &[],
            );
            Executor::new("rustup")
