...
pub fn start_monitor(port: u16) -> anyhow::Result<()> {
    let server = match Server::http(format!("0.0.0.0:{}", port)) {
        ...
        Err(_e) => {
            logging::warn(
                "WEB",
                &format!(
                    "Port {} in use, attempting fallback to {}...",
                    port,
                    port + 1
                ),
                &[],
    ...
    logging::status(
        "WEB",
        &format!("Build monitor live at http://localhost:{}", port),
    );
    ...
    for request in server.incoming_requests() {
        ...
        if url.starts_with("/dispatch/") {
            ...
            let _ = crate::engine::controller::UniversalController::dispatch_workflow(
                workflow,
                &crate::engine::ExecutionContext::from_defaults(),
            );
        ...
        let current_task = crate::utils::ui::logging::CURRENT_TASK
            .lock()
            .ok()
            .and_then(|t| t.clone())
