...
use super::{
    ModuleLoaderStats, ProcessPrepareError, BOOTSTRAP_TASK_ATTEMPTS,
    BOOTSTRAP_TASK_FAILURES, BOOTSTRAP_TASK_SUCCESS, LAST_PREFLIGHT_FINGERPRINT, MAP_PLAN_ATTEMPTS,
    MAP_PLAN_FAILURES, MAP_PLAN_SUCCESS, PARSE_ATTEMPTS, PARSE_FAILURES, PARSE_SUCCESS,
    PLAN_ATTEMPTS, PLAN_FAILURES, PLAN_SUCCESS, PREFLIGHT_ATTEMPTS, PREFLIGHT_FAILURES,
    PREFLIGHT_SUCCESS, SEGMENT_MATERIALIZATION_ATTEMPTS, SEGMENT_MATERIALIZATION_FAILURES,
    SEGMENT_MATERIALIZATION_SUCCESS, SEGMENT_MATERIALIZED_BYTES,
};
#[cfg(all(feature = "process_abstraction", feature = "paging_enable"))]
use super::VirtualMappingPlan;

#[cfg(all(feature = "process_abstraction", feature = "paging_enable"))]
pub fn materialize_virtual_mapping_range(
    start: u64,
    end: u64,
    prot: u32,
    ...
    page_manager
        .apply_virtual_mapping_plan(&[plan], flags.bits() as u32, frame_allocator)
...
fn prepare_bootstrap_runtime_entry(
    ...
    let entry = match super::prepare_process_image(process, image) {
        Ok(entry) => entry,
        Err(err) => {
            ...
            crate::klog_warn!(
                "[LOADER] prepare_process_image failed for bootstrap task: {:?}",
                err,
            );
