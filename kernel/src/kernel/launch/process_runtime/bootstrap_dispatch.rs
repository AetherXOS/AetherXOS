...
pub(super) fn prepare_aligned_static_dispatch(image: &'static [u8]) -> AlignedStaticDispatch {
    AlignedStaticDispatch {
        prepared_bootstrap: PreparedBootstrap {
            boot_image: BootImageRecord::BorrowedStatic(image),
        },
    }
}
