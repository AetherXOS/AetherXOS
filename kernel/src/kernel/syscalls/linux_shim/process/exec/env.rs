...
pub(super) fn normalize_execve_envp(
    ...
    ensure_env_var(&mut envp, "TERM", "aethercore");
    ensure_env_var(&mut envp, "LD_LIBRARY_PATH", "/lib:/usr/lib");
    ensure_env_var(&mut envp, "AETHERCORE_EXEC_PATH", path);
    ensure_env_var(
        &mut envp,
        "AETHERCORE_LINUX_RELEASE",
...
pub(super) fn append_exec_runtime_env(
    ...
    ensure_env_var(envp, "AETHERCORE_PHDR", &snapshot.phdr_addr.to_string());
    ensure_env_var(envp, "AETHERCORE_VDSO_BASE", &snapshot.vdso_base.to_string());
    ensure_env_var(envp, "AETHERCORE_VVAR_BASE", &snapshot.vvar_base.to_string());
    ensure_env_var(envp, "AETHERCORE_INIT_HOOKS", &init_hook_count.to_string());
    ensure_env_var(envp, "AETHERCORE_FINI_HOOKS", &fini_hook_count.to_string());
    ensure_env_var(envp, "AETHERCORE_EXEC_PATH", snapshot.exec_path.as_str());
    ensure_env_var(
        envp,
    ...
    ensure_env_var(
        envp,
        "AETHERCORE_SYSCALL_ABI_MAGIC",
        &crate::kernel::syscalls::SYSCALL_ABI_MAGIC.to_string(),
    );
    ensure_env_var(envp, "AETHERCORE_SYSCALL_PLATFORM", execve_aux_platform());
    ensure_env_var(
        envp,
    ...
    ensure_env_var(
        envp,
        "AETHERCORE_RUNTIME_VVAR",
        bool_env_value(snapshot.vvar_base != 0),
    );
    ensure_env_var(envp, "AETHERCORE_RUNTIME_TLS", bool_env_value(true));
    ensure_env_var(envp, "AETHERCORE_RUNTIME_SIGNAL_ABI", bool_env_value(true));
    ensure_env_var(
        envp,
    ...
    ensure_env_var(envp, "AETHERCORE_STARTUP_CRT0", "1");
    ensure_env_var(envp, "AETHERCORE_STARTUP_ARGV_ENVP", "1");
    ensure_env_var(envp, "AETHERCORE_STARTUP_AUXV", "1");
    ensure_env_var(envp, "AETHERCORE_STARTUP_RUNTIME_CONTRACT_ENV", "1");
    ensure_env_var(envp, "AETHERCORE_ELF_CLASS", "ELF64");
    ensure_env_var(envp, "AETHERCORE_ELF_ENDIANNESS", "little");
    ensure_env_var(envp, "AETHERCORE_ELF_MACHINE_TARGETS", "x86_64,aarch64");
    ensure_env_var(
        envp,
        "AETHERCORE_ELF_LOADER_FEATURES",
    ...
    ensure_env_var(
        envp,
        "AETHERCORE_ELF_RELOCATION_FAMILIES",
    ...
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_BASE", "7");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_PHDR", "3");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_PHENT", "4");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_PHNUM", "5");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_PAGESZ", "6");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_ENTRY", "9");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_UID", "11");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_EUID", "12");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_GID", "13");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_EGID", "14");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_PLATFORM", "15");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_HWCAP", "16");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_CLKTCK", "17");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_SECURE", "23");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_RANDOM", "25");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_EXECFN", "31");
    ensure_env_var(envp, "AETHERCORE_AUXV_AT_SYSINFO_EHDR", "33");
    ensure_env_var(
        envp,
