...
pub fn check_signals(frame: &mut SyscallFrame) {
    ...
    ))]
    {
        ...
        // Hardening: Validate user stack range before writing
        if !crate::kernel::syscalls::user_writable_range_valid(sp as usize, core::mem::size_of::<LinuxRTFrame>()) {
            crate::klog_warn!("[SIGNAL] Invalid user stack {:#x} for sig delivery", sp);
            return;
        }
    ...
    ))]
    {
        ...
        // Hardening: Validate user stack range before writing
        if !crate::kernel::syscalls::user_writable_range_valid(sp as usize, core::mem::size_of::<LinuxRTFrameCompat>()) {
            crate::klog_warn!("[SIGNAL SHIM] Invalid user stack {:#x} for sig delivery", sp);
            return;
        }
