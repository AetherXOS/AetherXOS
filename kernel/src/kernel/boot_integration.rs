//! Boot infrastructure integration.
//!
//! Wires boot manager, subsystems, and device manager into kernel boot stages.
//! This module sits in `kernel/` to avoid a circular dependency between
//! `kernel_runtime` and `kernel` (both previously referenced each other).
...
pub fn initialize_runtime_extensions() -> Result<(), &'static str> {
    ...
    crate::kernel_runtime::memory_integration::init_memory_extensions()?;
