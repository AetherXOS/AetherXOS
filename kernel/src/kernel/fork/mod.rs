...
use context::snapshot_parent_task;
...
pub fn do_fork(
    ...
    // ── 3. Clone Address Space (COW) ───────────────────────────────────────
    #[cfg(feature = "paging_enable")]
    let child_cr3 = crate::kernel::vmm::clone_current_address_space()
        .map_err(|_| ForkError::RegistryFull)?; // Use EAGAIN for OOM
    ...
    // ── 5. Create child Process (inherits security / limits / namespace) ────
    #[cfg(feature = "paging_enable")]
    let mut child_proc = crate::kernel::process::Process::new_with_cr3(
        &child_name,
        x86_64::PhysAddr::new(child_cr3),
    );
    #[cfg(not(feature = "paging_enable"))]
...
pub fn do_exec(pid: ProcessId, new_entry: u64) -> Result<(), ForkError> {
    ...
    if let Some(task_arc) = get_task(primary_tid) {
        ...
        {
            let _ = tls::initialize_task_tls(&process, &mut task);
            #[cfg(target_arch = "x86_64")]
...
pub fn do_clone(
    ...
    #[cfg(all(feature = "ring_protection", feature = "posix_mman"))]
    if tls_ptr == 0 {
        let _ = tls::initialize_task_tls(&process, &mut new_task);
    }
