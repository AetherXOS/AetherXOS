...
pub(super) struct PlatformBaseStatus {
    ...
    pub iommu_ready: bool,
    pub iommu_backend: &'static str,
    pub cpu_count: usize,
    pub ap_online: u32,
...
#[inline(always)]
pub(super) fn classify_platform(acpi_present: bool, hypervisor_present: bool) -> PlatformKind {
    if hypervisor_present {
        PlatformKind::VirtualMachine
    } else if acpi_present {
        PlatformKind::Pc
    } else {
...
pub(super) fn virt_platform_status(
    ...
    let memory_isolation_ready = iommu.initialized && iommu.hardware_mode && hardware_accel;
    let backend = crate::hal::common::virt::backend_name(virt);
    let ops = operations::current_operation_support(virt, iommu, memory_isolation_ready);
    let detail = crate::hal::x86_64::virt::detail::summarize(
        virt,
        memory_isolation_ready,
        iommu.attached_devices,
    );
    let backend_support =
        backend::current_backend_support(backend, hardware_accel, iommu, x2apic_enabled);
    let runtime = runtime::current_runtime_support();
    let lifecycle = crate::hal::x86_64::virt::guest_backend_state_machine();
