...
pub(super) fn apply_arch_switch_state(
    cpu: &hypercore::kernel::cpu_local::CpuLocal,
    ...
    #[cfg(all(feature = "ring_protection", target_arch = "x86_64"))]
    {
        ...
        if CORE_ENABLE_TLS_SYSCALLS {
            hypercore::hal::cpu::ArchCpuRegisters::write_tls_base(switch_info.next_tls);
        }

        let current_cr3 = hypercore::hal::cpu::ArchCpuRegisters::read_page_table_root();
        if current_cr3 != switch_info.next_cr3 {
            hypercore::hal::cpu::ArchCpuRegisters::write_page_table_root(switch_info.next_cr3);
        }
    }
}
