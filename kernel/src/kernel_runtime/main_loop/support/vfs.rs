...
pub(crate) fn service_vfs_runtime() {
    ...
    let sample = super::super::state::VFS_SLO_SAMPLE_COUNTER
        .fetch_add(1, Ordering::Relaxed)
        .wrapping_add(1);

    if super::is_sample_boundary(sample, super::super::state::VFS_SLO_SAMPLE_INTERVAL) {
        ...
        if report.breach_count > 0 {
            super::super::state::VFS_SLO_BREACH_STREAK.fetch_add(1, Ordering::Relaxed);
        } else {
            super::super::state::VFS_SLO_BREACH_STREAK.store(0, Ordering::Relaxed);
        }

        let last_log = super::super::state::VFS_SLO_LAST_LOG_SAMPLE.load(Ordering::Relaxed);
        let should_log_now = super::should_log_now(
            sample,
            super::super::state::VFS_SLO_SAMPLE_INTERVAL,
            last_log,
            super::super::state::VFS_SLO_LOG_INTERVAL_MULTIPLIER,
        );

        if report.breach_count > 0 && should_log_now {
            super::super::state::VFS_SLO_LAST_LOG_SAMPLE.store(sample, Ordering::Relaxed);
        ...
        let streak = super::super::state::VFS_SLO_BREACH_STREAK.load(Ordering::Relaxed);
        if streak >= super::super::state::VFS_SLO_ACTION_STREAK_THRESHOLD {
            match aethercore::modules::vfs::recommended_mount_health_action(report) {
                aethercore::modules::vfs::VfsMountHealthAction::PreferUnbufferedIo => {
                    if aethercore::config::KernelConfig::vfs_enable_buffered_io() {
                        ...
                        super::super::state::VFS_SLO_POLICY_ACTIONS.fetch_add(1, Ordering::Relaxed);
                        aethercore::klog_warn!(
                            "[VFS SLO] policy action: prefer_unbuffered_io (actions={})",
                            super::super::state::VFS_SLO_POLICY_ACTIONS.load(Ordering::Relaxed)
                        );
                    }
                }
