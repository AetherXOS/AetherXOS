...
pub(super) fn init_storage_drivers(
    ...
    aethercore::modules::drivers::StorageManager::init_global(devices);
    let global_lock = aethercore::modules::drivers::StorageManager::global();
    let mut storage = global_lock.lock();
    let Some(storage) = storage.as_mut() else {
        aethercore::klog_warn!("StorageManager global state was not initialized");
        return;
    };
    ...
    let block_stats = aethercore::modules::drivers::block::stats();
