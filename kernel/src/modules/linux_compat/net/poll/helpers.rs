...
where
    F: FnOnce() -> usize,
{
    match run_with_temporary_sigmask_result(temp_mask, || Ok(op())) {
        Ok(v) => v,
        Err(e) => e,
    }
}
...
#[inline]
pub(super) fn run_with_temporary_sigmask_result<T, F>(temp_mask: Option<u64>, op: F) -> Result<T, usize>
where
