...
pub unsafe fn init_local_apic() {
    ...
    block.reg::<u32>(bits::LAPIC_SVR).write(enable_spurious_vector(svr));
    ...
    block.reg::<u32>(bits::LAPIC_LVT_TIMER).write(periodic_timer_vector(LAPIC_PERIODIC_TIMER_VECTOR));
...
unsafe fn calibrate_apic_timer(block: &MmioBlock) -> u32 {
    ...
    block.reg::<u32>(bits::LAPIC_TICR).write(APIC_TIMER_MAX_INITIAL_COUNT);
    ...
    if elapsed == 0 || budget == 0 { APIC_CALIBRATION_FALLBACK_TICKS } else { elapsed }
}

pub unsafe fn eoi() {
    get_lapic_block().reg::<u32>(bits::LAPIC_EOI).write(0);
}

pub unsafe fn id() -> u32 {
    get_lapic_block().reg::<u32>(bits::LAPIC_ID).read() >> 24
}
...
pub unsafe fn send_ipi_all_excluding_self(vector: u8) {
    let block = get_lapic_block();
    block.reg::<u32>(bits::LAPIC_ICR_LOW).write(broadcast_excluding_self_icr(vector));
}
...
pub unsafe fn init_x2apic() {
    ...
    cpu::write_msr(X2APIC_LVT_TIMER, periodic_timer_vector(LAPIC_PERIODIC_TIMER_VECTOR) as u64);
    let ticks = calibrated_ticks_or_default(CALIBRATED_TICKS);
    cpu::write_msr(X2APIC_TICR, ticks as u64);
}

pub unsafe fn x2apic_eoi() { cpu::write_msr(X2APIC_EOI, 0); }
pub unsafe fn x2apic_id() -> u32 { cpu::read_msr(X2APIC_ID) as u32 }

pub unsafe fn x2apic_send_ipi(apic_id: u32, vector: u8) {
    cpu::write_msr(X2APIC_ICR, x2apic_ipi_command(apic_id, vector));
}

pub unsafe fn x2apic_send_ipi_all_excluding_self(vector: u8) {
    cpu::write_msr(X2APIC_ICR, x2apic_broadcast_ipi_command(vector));
}
