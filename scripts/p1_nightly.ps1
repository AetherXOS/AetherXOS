param(
    ...
    [string]$OutDir = "reports/p1_nightly",
    [int]$SoakRounds = 80,
...
Write-Host ""
Write-Host "==> P1 Nightly" -ForegroundColor Cyan
...
$cmd = @(
    ...
    "--max-avg-duration-increase-pct", "30",
    "--max-p95-duration-increase-pct", "40",
    "--max-max-duration-increase-pct", "55",
    ...
    "--max-avg-vs-trend-increase-pct", "35",
    "--max-p95-vs-trend-increase-pct", "45",
    "--max-failure-rate-vs-trend-increase-pctpoint", "0",
    "--out-dir", "$OutDir"
)
...
Write-Host "P1 nightly completed." -ForegroundColor Green
